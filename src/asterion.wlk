...
class ChestMimic inherits Enemigo { //Quise hacer que este herede de dos clases pero no tengo la forma aun por que es un chest pero tambien un enemigo

	var property posicion
	var property estado = cerrado
	var property drop // aca no uso artefacto a dropear por que si no deberia cambiar la de cada enemigo cuando lo instancio y cambiar en clase que no dropee aire por defecto
	
	override method artefactoADropear() = self.drop()
	
	override method vidaBase(){
		return 40
	}
	...
	override method esArtefacto() {
		return self.estaCerrado()
	}
